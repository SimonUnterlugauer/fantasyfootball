...
for row in rows[1:]:
    ...
    if len(columns) >= 25:
        ...
        average_ftp = int(fantasy_points_overall) / int(games_played) if int(games_played) != 0 else 0

        # Insert the extracted data into the table
        insert_query = '''
            INSERT INTO fantasy_data (position, team_name, passes, player_name, games_played, fantasy_points_overall, players_vbd, average_points)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        '''
        data = (position, team_name, passes, player_name, games_played, fantasy_points_overall, players_vbd, average_ftp)
